...
const getFormJsonFromApi = async (message: any) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const formGetUrl = `https://www.formstack.com/api/v2/form/${formId}/confirmation.json`;
...
class ApiConfirmationEmailManager {
  static #instance: ApiConfirmationEmailManager;
  #cacheDirectory = process.cwd() + '/api-cache/confirmation-email';
  ...
  static getInstance(): ApiConfirmationEmailManager {
    if (!ApiConfirmationEmailManager.#instance) {
      ApiConfirmationEmailManager.#instance = new ApiConfirmationEmailManager();
    }
    return ApiConfirmationEmailManager.#instance;
  }
}

export { ApiConfirmationEmailManager };
